...
$sort_query = "select id,date,likes,comments,shares from feeds where (date between '".date("Y-m-01")."' and '".date("Y-m-d")."' and group_id = '".$page_id."') order by likes DESC";
...
-->
	<div style="font-size:20px">
		...
		<a href="./group_tool_insight.php">社團個別洞察分析</a>
		<span class="glyphicon glyphicon-arrow-right" aria-hidden="true">
		</span>
		<?php echo '<a href="./group_insight.php?id='.$page_id.'">'.$page_list[$page_id].'</a>';?>
	</div>
...
<br>
<div class="well well-sm">動態牆 : 您可以透過以下區塊了解此社團最新與最熱門之動態文章並與上方圖表進行交叉比對</div>
        ...
        <div role="tabpanel" class="tab-pane fade in active" id="hot">
            <?php
                foreach($hot_posts_arr as $post_id){
                    echo '<div class="fb-post" data-href="https://www.facebook.com/'.$page_id.'/feeds/'.$post_id.'" data-width="500"></div>';
                }
            ?>
        </div>
        <?php  
